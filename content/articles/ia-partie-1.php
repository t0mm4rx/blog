...
Cette première partie est centrée sur le <b>fonctionnement théorique d'un seul neurone</b>, qui constitue la base d'un réseau de neurones. Un neurone seul peut, nous le verrons, résoudre des problèmes simples. Après les explications théoriques je vous montrerai dans la partie 2 comment implémenter un neurone en Javascript, et nous résoudrons un premier problème grâce au <b>machine learning</b>.
...
Cet article se veut destiné aux débutants, certains détails seront simplifiés afin de rendre les explications les plus claires possible.</p>
<h3>À quoi sert un réseau de neurones ?</h3>
<p>Un réseau de neurones à un but précis : <b>prédire un résultat à partir de valeurs données</b>. Voici une liste d'exemple :
<ul>
...
Dans touts ces cas, le réseau prend en entrée des données, et sort une nouvelle valeur.
</p>
...
Reprenons l'exemple du spam : votre messagerie doit détecter si un mail reçu est un spam ou non. Il va constituer une base de données de mails spam signalés par les utilisateurs. Grâce à cette base de mails spam il peut entrainer son réseau de neurones à détecter de nouveaux cas inconnus. <b>Un réseau de neurones crée une règle générale à partir de données existantes afin de pouvoir appliquer cette règle à de nouveaux cas</b>.</p>
...
Imaginez une boite avec des potars qui correspondent au poids. Le machine learning c'est tourner ces boutons dans tous les sens pour trouver une configuration qui donne le résultat le plus satisfaisant possible.<br/>
...
<p>Nous allons maintenant aborder une partie cruciale dans le fonctionnement d'un réseau de neurones : l'apprentissage. Le machine learning consiste à donner une grande quantité d'exemples issus d'un <b>dataset</b>, un large ensemble de données, afin de trouver les poids qui donne un résultat le plus juste possible. Autrement dit, <b>il faut minimiser l'erreur d'un neurone</b>. Pour chaque donnée d'entrainement, le neurone va ajuster ses poids pour se rapprocher du résultat attendu. Voici le procédé d'apprentissage :
<ul>
  <li>
  ...
  <li>
    ...
    Il est important de préciser qu'il existe différents type de régressions, la plus simple étant la linéaire mais il en existe d'autres comme la régression logistique. Dans notre cas la régression linéaire nous suffira largement.
  </li>
  <li>
